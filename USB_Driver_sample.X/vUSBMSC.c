...
//UINT32	*BD;        // BD array access pointer
...
void __ISR(_USB_1_VECTOR, IPL1AUTO) _USB1Interrupt (  )
{
...
//	BDT_IN.ADR = (UINT16)UsbBufDAT512;	// UsbBufDAT512 is data buffer
	BDTs.BDT_IN_D0.ADR = (UINT32)UsbBufCMD64;	// Setup IN BD buffer address (max 64 bytes). *Command buffer.
	BDTs.BDT_IN_D1.ADR = (UINT32)UsbBufCMD64;	// Setup SETUP/OUT BD buffer address (max 64 bytes) *Command buffer.
	BDTs.BDT_OUT_D0.ADR = (UINT32)UsbBufCMD64;	// Setup SETUP/OUT BD buffer address (max 64 bytes) *Command buffer.
	BDTs.BDT_OUT_D1.ADR = (UINT32)UsbBufCMD64;	// Setup SETUP/OUT BD buffer address (max 64 bytes) *Command buffer.
	...
	U1BDTP3 = ((UINT32)BDT >> 24) & 0xff;	// Set BDT address 
	U1BDTP2 = ((UINT32)BDT >> 16) & 0xff;	// Set BDT address 
	U1BDTP1 = ((UINT32)BDT >> 8) & 0xfe;	// Set BDT address 
...
void USBMSC_statusControl(void)
{
	...
	case  	eUSB_SETUP_start:
        ...
        BDT_OUT.ADR = (UINT32)UsbBufCMD64;	// SET_CONFIG request packet address to BDT_OUT buffer address.
        BDT_OUT.STAT.UOWN = 1;
        BDT_OUT.count = 8;	// Setup BD to send 8 byte
...
//        BDT_IN.UOWN_COUNT = 0x80FF;	// Setup BD to get 255 byte
        BDT_IN.STAT.UOWN = 1;
        BDT_IN.count = 255;	// Setup BD to send 255 byte
        BDT_IN.STAT.DTS  = 1;
	...
	case  	eUSB_BULK_dataIN_start:
        ...
        BDT_OUT.STAT.UOWN = 1;
        BDT_OUT.count = 64;	// Setup BD to send 64 byte
