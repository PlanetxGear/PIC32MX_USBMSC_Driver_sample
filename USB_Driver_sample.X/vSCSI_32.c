...
const UINT8 SCSIinquiry[] = {	// INQUIRY command contents(SCSI)
	...
	0x10, 0x00, 0x00, 0x00, 0x80, 0x00, 0x0c, 0x12,
...
const UINT8 SCSIreadFormatCapacity[] = {	// READ FORMAT CAPACITY command contents(SCSI)
	0x55, 0x53, 0x42, 0x43, 0x01, 0x00, 0x00, 0x00,
	0x08, 0x00, 0x00, 0x00, 0x80, 0x00, 0x0c, 0x23,
...
enum eSCSI_STATE SCSI_isErrInCSW(INT16 dataSize, UINT8 buf[])
{
    ...
    {
        DEBUG_PRINTF("SCSI_ANY_ERROR,code 0x:%02x\n",(UINT8)buf[12]);
...
void SCSI_statusControl(void)
{
	...
	case  	eSCSI_CBW_start :
		...
		USBcondition.BufferAddress = (UINT32)UsbBufCMD64;	// the command buffer address to BDT_OUT buffer address.
        USBcondition.SendBytes = 31;	// Setup BD to send 31 byte. It must be appropriate data size.
//        BDT_OUT.STAT.UOWN = 1;
//        BDT_OUT.UOWN_COUNT = 0x801f;	// Setup BD to send 31 byte. It must be appropriate data size. 
	...
	case  	eSCSI_getData_start :
		...
		USBcondition.BufferAddress = (UINT32)SCSIcondition.UsbBuffAddr;	// the data buffer address to BDT_IN buffer address.
        USBcondition.SendBytes = 64;	// Setup BD to send 31 byte. It must be appropriate data size.
		setUSB_DATAIN();
		SCSIcondition.Status++;		//next step
		break;
    ...
    case  	eSCSI_putData_start :
		...
		USBcondition.BufferAddress = (UINT32)SCSIcondition.UsbBuffAddr;	// the data buffer address to BDT_OUT buffer address.
        USBcondition.SendBytes = 64;	// Setup BD to send 64 byte. It must be appropriate data size. 
//        BDT_OUT.STAT.UOWN = 1;
//        BDT_OUT.UOWN_COUNT = 0x8040;	// Setup BD to send 64 byte. It must be appropriate data size. 
	...
	case  	eSCSI_CSW_start :
		...
		USBcondition.BufferAddress = (UINT32)UsbBufCMD64;	// the command buffer address to BDT_IN buffer address.
        USBcondition.SendBytes = 64;	// Setup BD to send 64 byte. It must be appropriate data size. 
		setUSB_DATAIN();
		SCSIcondition.Status++;		//next step
		break;
...
)
{
    ...
    SCSIcondition.Command = eSCSI_inquiry;
...
int	i;
    ...
    if(SCSIcondition.Status > eSCSI_ERRORS )
    {
        // if there is the error default setting as 0x1ffff; 
        SCSIcondition.MscTotal = 0x1ffff;
    }
    else
    {
        ...
        {
            SCSIcondition.MscTotal = (SCSIcondition.MscTotal << 8) + *(buffAddr + i);	// Get 32bit number
        }
...
int	i;
    ...
    else
    {
        ...
        {
            SCSIcondition.DataLength = (SCSIcondition.DataLength << 8) + *(buffAddr + i);	// Get 32bit number
        }
    }
    DEBUG_SCSI2PRINTF("LastSecter:0x%08lx DataLength:0x%08lxbyte\n",SCSIcondition.MscTotal, SCSIcondition.DataLength);
